...
async function getProof({
  ...
  for (const input of inputs) {
    if (input.amount > 0) {
      ...
      inputMerklePathIndices.push(input.index.toString())
      inputMerklePathElements.push(tree.path(input.index).pathElements.map( x => BigNumber.from(x).toString()))
      if (input.index != 0 && input.amount > 0){
        console.log("Amount", input.amount);
        console.log("Non zero merkle proof:");
        console.log("let index: u128 = ", input.index, ";");
        console.log("let path_elements = ", JSON.stringify(utils.stringifyBigInts(tree.path(input.index).pathElements)), ";");
        console.log("let leaf = \"", input.getCommitment().toString(), "\";");
        console.log("let expected_root = \"", tree.root().toString(), "\";");
      }
    } else {
  ...
  let input2 = {
    ...
    input_nullifier: inputs.map((x) => x.getNullifier().toString()),
    output_commitment: outputs.map((x) => x.getCommitment().toString()),
    public_amount: BigNumber.from(extAmount).sub(fee).add(FIELD_SIZE).mod(FIELD_SIZE).toString(),
    ext_data_hash: extDataHash.toString(),

    // data for 2 transaction inputs
    in_amount: inputs.map((x) => x.amount.toString()),
    in_private_key: inputs.map((x) => BigNumber.from(x.keypair.privkey).toString()),
  ...
  console.log("input0 pubkey = ", inputs[0].keypair.pubkey.toHexString());
  console.log("input1 pubkey = ", inputs[1].keypair.pubkey.toHexString());
